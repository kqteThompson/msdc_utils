        ...
        if 'pick' in action:
            ...
            for color in colors:
                if color in worldstatedict.keys():
                    if coord in worldstatedict[color]:
                        worldstatedict[color].remove(coord)
                        break
        else:
            for color in colors:
                if color in action:
                    coord = action.split(color)[1].strip()
                    worldstatedict[color].append(coord)
...
def format_worldstate(worldstatedict):
    ...
    for key in worldstatedict.keys():
        if len(worldstatedict[key]) > 0: #don't print info about 'empty' colors
            num = str(len(worldstatedict[key]))
            num += ' ' + key + ' '
            coords = []
            coords.extend([coord for coord in worldstatedict[key]])
    ...
    if len(state) > 1:
        ...
        state_str = 'Empty' #If all block colors are empty
    return state_str
...
with open(current_folder + '/test.json', 'w') as outfile:
