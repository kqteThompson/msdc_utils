...
example: 
<last moves from previous narration> + <WORLDSTATE at beginning of the narration> + Inst =>  ACTION
if more than one action:
<last moves from previous narration> + <WORLDSTATE at beginning of the narration> + <Inst> + <action> + <Inst> => ACTION
TRAIN
longest field = 3007
3722 train samples
VAL
...
train_data = current_folder + '/test.json'
...
with open(current_folder + '/actseq-test-narr-V3.csv', 'w') as f:
    write = csv.writer(f)
