...
def rectangle_clarifq(params_list):
    ...
    #chose one param at random
    params = [k for k in params_list.keys() if k not in ['shape']] 
    rand = random.choice(params)
    if rand == 'size':
        if LOC:
            instruction = f"<Architect> Build an {params_list['col']} {params_list['orient']} {SHAPE} at the {params_list['loc']}.".replace("  "," ")
        else:
            instruction = f"<Architect> Build an {params_list['col']} {params_list['orient']} {SHAPE}.".replace("  "," ")
        clarifq = f"<Builder> What size?".replace("  "," ")
        answer = f"<Architect> {params_list['size']}.".replace("  "," ")

    elif rand == 'col':
        if LOC:
            instruction = f"<Architect> Build an {params_list['size']} {params_list['orient']} {SHAPE} at the {params_list['loc']}.".replace("  "," ")
        else:
            instruction = f"<Architect> Build an {params_list['size']} {params_list['orient']} {SHAPE}.".replace("  "," ")
        clarifq = f"<Builder> What color {SHAPE}?".replace("  "," ")
        answer = f"<Architect> {params_list['col']}."

    elif rand == 'orient':
        if LOC:
            instruction = f"<Architect> Build an {params_list['col']} {params_list['size']} {SHAPE} at the {params_list['loc']}.".replace("  "," ")
        else:
            instruction = f"<Architect> Build an {params_list['col']} {params_list['size']} {SHAPE}.".replace("  "," ")
        clarifq = f"<Builder> Horizontal or vertical?".replace("  "," ")
        answer = f"<Architect> {params_list['orient']}."

    elif rand == 'loc':
        instruction = f"<Architect> Build an {params_list['col']} {params_list['size']} {params_list['orient']} {SHAPE}.".replace("  "," ")
...
def clarif_question(params_list):
    ...
    if rand == 'size':
        if SHAPE in ['row', 'diagonal']:
            ...
            clarifq = f"<Builder> What size {SHAPE}?".replace("  "," ")
        ...
        if SHAPE in ['row', 'diagonal']:
            if LOC:
                if LOC == 'centre':
                    ...
                    instruction = f"<Architect> Build a {params_list['orient']} {SHAPE} of {params_list['size']} blocks at the {params_list['loc']}.".replace("  "," ")
            else:
                instruction = f"<Architect> Build a {params_list['orient']} {SHAPE} {params_list['size']} blocks long.".replace("  "," ")
            clarifq = f"<Builder> What color {SHAPE}?".replace("  "," ")
        ...
        if SHAPE in ['row', 'diagonal']:
            instruction = f"<Architect> Build a {params_list['orient']} {params_list['col']} {SHAPE} {params_list['size']} blocks long.".replace("  "," ")
            clarifq = f"<Builder> Where?"
            if LOC:  
                if LOC == 'centre':
    ...
    ## !! will only be for diamonds, rectangles and squares
        ...
        clarifq = f"<Builder> Horizonal or vertical?"
