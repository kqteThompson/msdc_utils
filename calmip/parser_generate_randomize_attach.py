...
def add_previous(sample, previous, predictions):
    """
    in this randomize version of generation, we return the head
    so we how the window for randomizing the source indices
    """
    new_output = []
    keep_str = None
    #get head
    ...
    for s in slist:
        ...
        new_output.append(s)
    new_output_string = '\n'.join(new_output)
    return keep_str, new_output_string, head

def format_gen(preds, head):
    """
    this version takes the head index for randomizing structure
    """
    ...
    for a in split_list:
        ...
        if rel != None and s_tuple != None:
            target = s_tuple[1]
            ##Here make random source decision
            source_options = list(range(head, target))
            new_source = random.choice(source_options)
            ##Here inject a random label
            new_labels = [l for l in labels if l != rel]
            randrel = random.choice(new_labels)
            clean_list.append(randrel + '(' + str(new_source) + ',' + str(target) + ')')
...
f = open("/tmpdir/thompson/llama-trois-parser/test-output-generate-file-llama3-randomize-attach.txt","w")

new_generations = None
previous_generations = None
head = 0
for datum in tqdm(test_dataset['sample'][:5]):
    ...
    if is_first_moves(datum):
        ...
        update_prev, amended_text, head = add_previous(datum, previous_generations, new_generations)
    ...
    new_generations = format_gen(generated.split('### DS:')[1], head)
