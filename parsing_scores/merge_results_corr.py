...
# print("double rels: ", doubles)
    ...
    if len(gold_all) > 0 and len(pred_all) > 0: ##!! there is an issue here: when Pred is none but gold is not, we still don't rec tru pos
    ...
    print("gold all", gold_all)
    print("pred all", pred_all)
    total_TP.extend(TP)
    total_FN.extend(FN)
    total_FP.extend(FP)
