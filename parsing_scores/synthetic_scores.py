...
gold_path = current_folder + '/synthetic/synthetic_corrections_test.jsonl'
pred_path = current_folder + '/synthetic/synthetic_corrections_test_216.txt'
...
type_one = 0 ##first instruction botched 
type_two = 0 ##second instruction botched 
correct_type_one = 0
correct_type_two = 0
...
for i, s in enumerate(pred_outputs):
    ...
    if len(gold_all) == 1:
        if gold_all[0][1] == 4: 
            type_two += 1   
            assert len(pred_all) == 1
            assert pred_all[0][0] == 'CORR'
            assert pred_all[0][2] == 5
            if pred_all[0][1] != 4: 
                assert pred_all[0][1] == 2
            else:
                correct_type_two += 1
        elif gold_all[0][1] == 2: 
            type_one += 1
            ...
            assert pred_all[0][2] == 5
            if pred_all[0][1] != 2: 
                assert pred_all[0][1] == 4
            else:
                correct_type_one += 1
    elif len(gold_all) == 2:
        assert len(pred_all) == 2
        if gold_all[0] != pred_all[0]:
            assert gold_all[0] == pred_all[1]

        # print(gold_all)
        # print(pred_all)
        # print('-------')
        # assert gold_all == pred_all
    
print('num type one: ', type_one, 'num type two: ', type_two, 'correct type one: ', correct_type_one, 'correct type_two: ', correct_type_two)
